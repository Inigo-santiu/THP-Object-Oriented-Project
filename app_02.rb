...
def menu(player1, player2, player3)
  ...
  if player2.hit_points > 1
    puts "0 - to attack Hector the bull who has #{player2.hit_points}HP"
  end
  if player3.hit_points > 1
    puts "1 - to attack Dolly the cow who has #{player3.hit_points}HP"
  end
  ...
  case decision
    ...
    when "1"
      ...
      puts "You're enemies benefit from your indecision!" # Don't type nonsense.  Give a stupid answer, get a stupid result.
    end
  end
...
puts "This fight is over!"
  if player1.hit_points > 0      #This is the win condition.
    puts "You're a real cowboy YEEEEHAAAAAA!!"
  else
    puts "#{player1.name} has 0 HP remaining."  # This cannot happen unless you ask for it.
